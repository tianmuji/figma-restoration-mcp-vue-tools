...
### 4. 正确的节点ID识别流程
**关键步骤**: 在Phase 1分析阶段，必须通过Figma JSON分析找到正确的节点ID

#### 4.1 用户提供节点ID时的处理
当用户提供类似 `https://www.figma.com/design/...?node-id=9310-98365&m=dev` 的链接时：
1. **提取节点ID**: 从URL中提取 `9310:98365` (注意使用冒号分隔符)
2. **分析节点名称**: 在Figma JSON中查找该节点，确认其 `name` 属性
3. **判断节点类型**: 检查是否为容器节点还是具体素材节点
4. **选择正确层级**: 如果是容器，查找其子元素中的具体素材节点

#### 4.2 节点ID查找示例
```javascript
// 用户提供的链接: node-id=9310-98365
// 在Figma JSON中查找:
{
  "id": "9310:98365",
  "name": "新/cs_ic_ai_question_search_ai搜题",  // 这是容器节点
  "type": "INSTANCE",
  "componentId": "9350:79190",
  "children": [
    {
      "id": "I9310:98365;12982:842",  // 这才是具体的素材节点
